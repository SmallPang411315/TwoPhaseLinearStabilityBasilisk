...
scalar p[];//I add p_mean(SFD)
vector u[], u_mean[], g[];
scalar pf[];
face vector uf[];//used in vof.h
...
bool stokes = false, SFD_flag = false;//true is not considering

#if !SFD_flag
//double chi_sfd = 1., Delta_sfd = 1./0.5, t_sfd = 0.;//Cyl
double chi_sfd = 2.5, Delta_sfd = 1./0.5, t_sfd = 0.;//For jet
#endif
...
/**
We add the forcing terms to the centered velocity field. 
*/

void SFD_update_u (double chi_sfd, double Delta_sfd, double dt)
{
	foreach()
      foreach_dimension()
        u.x[] = 1.0/(1+chi_sfd*Delta_sfd)*(1.0+chi_sfd*Delta_sfd*pow(2.71828,-(chi_sfd+1.0/Delta_sfd)*dt))*u.x[]
		       +1.0/(1+chi_sfd*Delta_sfd)*(chi_sfd*Delta_sfd*(1-pow(2.71828,-(chi_sfd+1.0/Delta_sfd)*dt)))*u_mean.x[];
    boundary ((scalar *){u});  
}

void SFD_update_umean (double chi_sfd, double Delta_sfd, double dt)
{
	foreach()
      foreach_dimension()
        u_mean.x[] = 1.0/(1+chi_sfd*Delta_sfd)*(1.0-pow(2.71828,-(chi_sfd+1.0/Delta_sfd)*dt))*u.x[]
		            +1.0/(1+chi_sfd*Delta_sfd)*(chi_sfd*Delta_sfd+pow(2.71828,-(chi_sfd+1.0/Delta_sfd)*dt))*u_mean.x[];
    boundary ((scalar *){u_mean});  
}
...
event SFD (i++,last)
{
  if(!SFD_flag && t < t_sfd){
	foreach()
		foreach_dimension() u_mean.x[] = u.x[];
  }
	
  if (!SFD_flag && t > t_sfd) {
	  SFD_update_u (chi_sfd, Delta_sfd, dt);
	  SFD_update_umean (chi_sfd, Delta_sfd, dt);
  }
}
