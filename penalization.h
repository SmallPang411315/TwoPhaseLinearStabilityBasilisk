#ifndef BASILISK_HEADER_Pel
#define BASILISK_HEADER_Pel

const vector zerocf[] = {0.,0.,0.};
#ifdef BRINKMAN_PENALIZATION
    #define frhs (1 - fs[])*(1 - fs_jet[])
    //#define fbp (fs[])
    #define fbp (clamp(fs[]+fs_jet[],0,1))
    #define fbp_solid (fs[])
    extern scalar fs, fs_jet;
    ...
    (const) scalar a_br = unity, b_br = unity; // useful for Robin BC
                ...
                vector U_solid[], target_U[];
        (const) vector n_sol = zerocf;
        #define PLUS_BRINKMAN_RHS         + fbp_solid*dt*(- (u.x[] - target_U.x[])/eta_s)
        #define PLUS_NUMERATOR_BRINKMAN   + fbp_solid*dt*(sq(Delta)*target_U.x[]/eta_s)
        #define PLUS_DENOMINATOR_BRINKMAN + fbp_solid*dt*(sq(Delta)/eta_s)
    #elif BRINKMAN_PENALIZATION == 2 //Neumann BC
        vector target_U[], n_sol[];
        #define CALC_GRAD
        #define PLUS_BRINKMAN_RHS         + fbp_solid*dt*(nu_s*(u.x[1] - 2*u.x[] +u.x[-1])/sq(Delta) - (scalar_a_by_b(n_sol, grad_u) - target_U.x[])/eta_s)
        #define PLUS_NUMERATOR_BRINKMAN   + fbp_solid*dt*(nu_s*(u.x[1] + u.x[-1]) - (0.5*Delta*(m_scalar_a_by_b(n_sol, u)) - sq(Delta)*target_U.x[])/eta_s)
        #define PLUS_DENOMINATOR_BRINKMAN + fbp_solid*dt*(2*nu_s)
    #elif BRINKMAN_PENALIZATION == 3 //Robin BC
        vector target_U[], n_sol[];
        #define CALC_GRAD
        #define PLUS_BRINKMAN_RHS         + fbp_solid*dt*(nu_s*(u.x[1] - 2*u.x[] +u.x[-1])/sq(Delta) - (a_br[]*u.x[] + b_br[]*scalar_a_by_b(n_sol, grad_u) - target_U.x[])/eta_s)
        #define PLUS_NUMERATOR_BRINKMAN   + fbp_solid*dt*(nu_s*(u.x[1] + u.x[-1]) - (0.5*Delta*b_br[]*(m_scalar_a_by_b(n_sol, u)) - sq(Delta)*target_U.x[])/eta_s)
        #define PLUS_DENOMINATOR_BRINKMAN + fbp_solid*dt*(a_br[]*sq(Delta)/eta_s + 2*nu_s)
...
//        #define CALC_GRAD
//        #define PLUS_BRINKMAN_RHS         + fbp_solid*dt*(nu_s*(u.x[1] - 2*u.x[] +u.x[-1])/sq(Delta) - n_sol.x[]*scalar_a_by_b(n_sol, umUs)/eta_s)
//        #define PLUS_NUMERATOR_BRINKMAN   + fbp_solid*dt*(nu_s*(u.x[1] + u.x[-1]) - sq(Delta)*n_sol.x[]*(scalar_a_by_b(n_sol, umUs) - n_sol.x[]*u.x[])/eta_s)
//        #define PLUS_DENOMINATOR_BRINKMAN + fbp_solid*dt*(sq(n_sol.x[]*Delta)/eta_s + 2*nu_s)
    #else
...
//     if (!is_constant(U_solid.x)) foreach() {
//         target_U.x[] = U_solid.x[];
//         target_U.y[] = (fs_jet[]>0.5) ? 0*(y+2)/sqrt(sq(y+2)+sq(z)) : U_solid.y[];
//         target_U.z[] = (fs_jet[]>0.5) ? 0*(z)/sqrt(sq(y+2)+sq(z)) : U_solid.z[];
//     }
...
//     foreach() {U_solid.x[]=0.0;U_solid.y[]=0.;U_solid.z[]=0.;}
