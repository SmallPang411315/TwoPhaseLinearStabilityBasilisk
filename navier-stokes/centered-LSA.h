...
vector u[], Ubase[], g[];
scalar pf[];
face vector uf[], Ufbase[];//used in vof.h
...
(const) face vector mu = zerof, mu_base = zerof, a = zerof, alpha = unityf;
...
event stability (i++,last) {
  face vector UfbaseCoe[];
  foreach_face() UfbaseCoe.x[] = 1./rho2*Ufbase.x[];
  dt = dtnext (stokes ? dtmax : min(timestep (uf, dtmax), timestep (UfbaseCoe, dtmax)));
}
...
event advection_term (i++,last)
{
  if (!stokes) {
    ...
    advection ((scalar *){u}, Ufbase, dt, (scalar *){g});
    advection_linear((scalar *){u}, (scalar *){Ubase}, uf, dt, (scalar *){g});
    advection_linear_base((scalar *){u}, (scalar *){Ubase}, Ufbase, dt , (scalar *){g});
  }
}
...
event viscous_term (i++,last)
{
  if (constant(mu.x) != 0.) {
    correction (dt);
    mgu = viscosity (u, Ubase, mu, rho, dt, mgu.nrelax);
    mgu = viscosity_explicit_base (u, Ubase, mu_base, rho, dt, mgu.nrelax);
    correction (-dt);
  }
