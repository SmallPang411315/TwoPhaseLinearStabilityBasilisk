...
void input_grd_Phi (struct InputGRD p)
{
  scalar input = p.s;
  ...
  for (int i = ny - 1; i >= 0; i--)
    ...
    for (int i = ny - 1; i >= ny/2; i--)//symmetric
      for (int j = 0 ; j < nx; j++) {
        value[j + (ny-i-1)*nx] = value[j + i*nx];
    }
...
void input_grd_U (struct InputGRD p)
{
  scalar input = p.s;
...
void input_grd_V (struct InputGRD p)
{
  scalar input = p.s;
...
void input_grd_f (struct InputGRD p)
{
  scalar input = p.s;
