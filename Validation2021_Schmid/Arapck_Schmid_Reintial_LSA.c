...
int main() {
  ...
  a_int info = 0;//0 when no specific starting vector
...
{ 
  ...
  if (!restore (file = "Base", list = {Ubase.x,Ubase.y})) {
      ...
      input_grd_U(Ubase.x, file = "./U.asc");
      input_grd_V(Ubase.y, file = "./V.asc");
      if(numiter==0) input_grd_Phi(Phibase, file = "./Phi.asc");
      else input_grd_Phi(Phibase, file = "./Phibase_reinitialization.asc");
      input_grd_f(f, file = "./f.asc");
  ...
  if(numiter==0){
    ...
    FILE * fpPhi = fopen("Phibase_reinitialization.asc","w");
    output_grd(Phibase, fpPhi, 1./N, true, {{0,-2},{L0,2}});
    fclose(fpPhi);

    input_grd_Phi(Phibase, file = "./Phibase_reinitialization.asc");
    output_ppm (Phibase, file = "Phibase.png");
  }
...
event logPoint(i++){
  foreach(){
      if(fabs(x-L0/2.)<Delta && fabs(y-1.)<Delta && x>L0/2. && y>1.){
...
event output(t = 0; t += delta_t; t <= tend){
  vector  dPhibase[],dPhibasex[],dPhibasey[],dphi[],dphix[],dphiy[];
      boundary({Phibase});
...
//       dphi.x.gradient = minmod2;//我加了这俩明显发生变化了。。。
...
//Give primary variables to the tec_cc list. "cc" means "cell center".
  tec.tec_cc = list_copy({p, phi, dphi, Phibase, dPhibase, dPhibasex, dPhibasey, u, Ubase, dphix, dphiy});
//Give the name of variables to varname.
#if dimension>1
  sprintf(tec.varname,"x y p phi dphix dphiy Phibase dPhibase.x dPhibase.y dPhibasexx dPhibasexy dPhibaseyx dPhibaseyy u.x u.y Ubase.x Ubase.y dphixx dphixy dphiyx dphiyy");
#endif
